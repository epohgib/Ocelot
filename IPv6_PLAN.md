...
There is an `upstream/ipv6` branch (commit `4e7e250`, 2018) that attempts
the support, but it does not compile (`ip` vs `ip_tmp`), has a broken
`private_ipv6` (`|| true` discards every IPv6), applies `hex_decode` on
the IPs, and uses a data schema incompatible with the current master.
**Not a usable base.**
...
- Extend `db::record_peer` with parameters `const std::string &ipv4, const std::string &ipv6`
- Extend `db::record_snatch` the same way
...
ALTER TABLE users_main
  ADD COLUMN IF NOT EXISTS track_ipv6 TINYINT(1) NOT NULL DEFAULT 0;
```

(Put this in `gazelle.sql` or in `contrib/` if it exists.)
...
The current `peer_key` uses `peer_id + userid`. IPv6 is not part of the
key. Confirm this is fine: two peers of the same user on the same
torrent (same `peer_id`) still collide, IPv4 or IPv6.
